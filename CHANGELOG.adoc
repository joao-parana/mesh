...
icon:plus[] Search: A new search option `search.defaultFieldMappings` has been added. When disabled no default field mappings and fields will be added to the search index. Only those fields which specify a custom elasticsearch mapping will be added to the index. This is useful if you want to have finer control of what contents should be added to Elasticsearch. The setting is enabled by default. This way this change will not affect existing setups.
...
icon:plus[] Search: It is now possible to use encrypted and authenticated connections between Gentics Mesh and Elasticsearch. New settings and environment variables have been added. Details can be found in the link:{{< relref "elasticsearch.asciidoc" >}}#_security[Elasticsearch documentation] https://github.com/gentics/mesh/issues/759[#759]
